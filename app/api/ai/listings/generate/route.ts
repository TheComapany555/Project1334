...
export async function POST(req: NextRequest) {
  ...
  if (!body.context?.title?.trim()) {
    return NextResponse.json(
      {
        error:
          "Add a listing title (and any other details you have) before generating with AI.",
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const result = await generateListingContent({ context });
